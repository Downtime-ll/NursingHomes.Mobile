import React, {Image, Text, TouchableHighlight, TouchableOpacity, View} from 'react-native';
...
const AppList = React.createClass({
  render() {
    ...
    const {data, topBorder, onMorePress, onAppPress} = this.props;
      ...
      const appColList = row.map((col, colIndex) => {
        ...
        if (col.isMore) {
          ...
          <View key={'appCol-' + colIndex} style={styles.appCol}>
            <TouchableHighlight underlayColor='#B5B5B5' onPress={onMorePress}>
              <Text style={styles.appItemTextMore}>
                ...
              </Text>
            </TouchableHighlight>
          </View>
